...
def main():
    ...
    with st.sidebar:
        ...
        exercise_type = st.selectbox(
            ...
            ["Push-ups", "Squats"],
            help="Select the type of exercise in your video"
        )
    ...
    if 'processed' in st.session_state and st.session_state.processed:
        ...
        if results['processed_frames']:
            ...
            sample_frames = results['processed_frames'][::len(results['processed_frames'])//6][:6]
            ...
            for i, frame in enumerate(sample_frames):
                with cols[i % 3]:
                    ...
                    st.image(frame_rgb, caption=f"Frame {i*len(results['processed_frames'])//6}", use_container_width=True)
        ...
        if st.button("Download Annotated Video"):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as tmp_file:
                ...
                if results['processed_frames']:
                    ...
                    video_writer = cv2.VideoWriter(tmp_file.name, fourcc, 30, (width, height))
